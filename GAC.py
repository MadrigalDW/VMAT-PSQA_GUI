...
def getArcCheck(MRN, fieldIndexes, inputDate=''):
    ...
    try:
        ...
        try:
            QAfields[0]
            return "There are no PDFs in the patient file you searched. Please manually check the Mosaiq queries."
        except:
    ...
    try: # finds the most recent date
        ...
        if inputDate == '':
            for pathName in pdfList:
                ...
                if int(date[0:4]) > int(mostRecentDate[0:4]): # sees if year is more recent than current mostRecentDate
                    mostRecentDate = date
                elif int(date[0:4]) == int(mostRecentDate[0:4]): # if year is the same as mostRecentDate, sees if month is more recent
                    if int(date[5:7]) > int(mostRecentDate[5:7]):
                        mostRecentDate = date
                    elif int(date[5:7]) == int(mostRecentDate[5:7]): # if month is the same as mostRecentDate, sees if day is more recent
            ...
            for pathName in pdfList:
                ...
                if int(date[0:4]) > int(mostRecentDate[0:4]) and int(date[0:4]) <= int(inputDate[0:4]): # if year of date is more recent than mostRecentDate and earlier or the same as inputDate
                    if int(date[0:4]) < int(inputDate[0:4]): # if the year is earlier than inputDate, update mostRecentDate
                        mostRecentDate = date
                    else: # if it isn't, check the month
                        if int(date[5:7]) < int(inputDate[5:7]): # the year is later than mostRecentDate, same as inputDate, and the month is earlier than inputDate
                            mostRecentDate = date
                        elif int(date[5:7]) == int(inputDate[5:7]) and int(date[8:]) <= int(inputDate[8:]): # the year is later than mostRecentDate, same as inputDate, the month is the same as inputDate, and day is earlier
                            mostRecentDate = date
                elif int(date[0:4]) == int(mostRecentDate[0:4]) and int(date[0:4]) <= int(inputDate[0:4]): # year is same as mostRecent, earlier or same as inputDate
                    if int(date[5:7]) > int(mostRecentDate[5:7]): # month is more recent than mostRecent
                        if int(date[0:4]) < int(inputDate[0:4]): # year is older than inputDate
                            mostRecentDate = date
                        else:
                            if int(date[5:7]) < int(inputDate[5:7]): # month is older than inputDate
                                mostRecentDate = date
                            elif int(date[5:7]) == int(inputDate[5:7]):
                                if int(date[8:]) <= int(inputDate[8:]): # month is same as inputDate, day is same or earlier, and year is same and month is later than mostRecent
                                    mostRecentDate = date
                    elif int(date[5:7]) == int(mostRecentDate[5:7]) and int(date[5:7]) <= int(inputDate[5:7]): # month is same as mostRecent, earlier or same as inputDate
                        if int(date[8:]) > int(mostRecentDate[8:]) and (int(date[8:]) <= int(inputDate[8:] or int(date[5:7]) < int(inputDate[5:7]))): # same month and year but later day than mostRecent, month and day are earlier than or same as inputDate
                                mostRecentDate = date
    except:
