"""
Created on Tue May  1 15:17:02 2021
@author: weersim
"""

import clr, os, pyperclip, webbrowser, subprocess, glob
clr.AddReference("wpf\PresentationFramework")
from System.IO import StreamReader
from System import Uri, ComponentModel
from System.Windows import Window, LogicalTreeHelper, RoutedEventHandler, MessageBox#, Application
...
from mira_rest.mosaiq_qa import get_field_info, get_site_info #get_qa_records, 
...
class TestWindow(Window): 
    def __init__(self):
        ...
        stream = StreamReader('psqa_gui_clean.xaml') # expects a path name! 
        ...
        self.extraBoxinXAML = LogicalTreeHelper.FindLogicalNode(self.testwindow, "extraBox")
        self.siteBoxinXAML = LogicalTreeHelper.FindLogicalNode(self.testwindow, "typeBox")
        self.siteBoxinXAML.SelectionChanged +=  RoutedEventHandler(self.siteEntered)
        
        self.MRLinputboxinXAML = LogicalTreeHelper.FindLogicalNode(self.testwindow, "MRLinput")
        self.MRLinputboxinXAML.KeyDown +=  RoutedEventHandler(self.MRN_entered)
    ...
    def MRN_entered(self,sender,e):
        if e.Key == Key.Return:
            ...
            print('The MRN is ', self.MRNinputboxinXAML.Text, '\n')
            MRN = self.MRNinputboxinXAML.Text
            # thisis the main function that is called:
            self.getPDFImagesClick(sender, e, MRN)
    ...
    def teamEntered(self, sender, e):
        ...
        self.extraBoxinXAML.Items.Clear()
        self.extraBoxinXAML.IsEnabled = False
        self.MRLinputboxinXAML.IsEnabled = False
        if self.teamBoxinXAML.SelectedItem.ToString() == 'System.Windows.Controls.ComboBoxItem: Team 1':
            ...
            for item in itemList:
             ...
             self.MRLinputboxinXAML.IsEnabled = True
             self.extraBoxinXAML.IsEnabled = True
             self.extraBoxinXAML.Items.Add('Prostate, Bladder, Pelvic Oligomets')
             self.extraBoxinXAML.Items.Add('Liver, Pancreas, Kidney')
    ...
    def siteEntered(self, sender, e):
        ...
        if self.siteBoxinXAML.SelectedItem == 'Complex Palliative':
            self.extraBoxinXAML.Items.Add('Complex Palliative')
            self.extraBoxinXAML.Text = 'Complex Palliative'
            
        elif self.siteBoxinXAML.SelectedItem == 'Oligomets':
            self.extraBoxinXAML.Items.Add('Oligomets')
            self.extraBoxinXAML.Text = 'Oligomets'
        else:
            if self.teamBoxinXAML.SelectedItem.ToString() == 'System.Windows.Controls.ComboBoxItem: Team 2':
                ...
                for item in criteriaList:
                    self.extraBoxinXAML.Items.Add(item)
    ...
    def getPDFImagesClick(self, sender, e, MRN):
        ...
        try:
            if self.teamBoxinXAML.Text == 'Team MRL':
                m = Measurements2(MRN, self.MRLinputboxinXAML.Text)
                print('Team MRL')
            else:
                m = Measurements(MRN, self.siteBoxinXAML.Text)
            #breakpoint()
            self.mDictionary = m.dictionary # why? 
        ...
        if self.teamBoxinXAML.Text == 'Team 2' or self.teamBoxinXAML.Text == 'Team 3' or self.teamBoxinXAML.Text == 'Team MRL':
            if self.extraBoxinXAML.Text == '':
                MessageBox.Show('QA criteria has not been selected. Minimum passrates may not be accurate.')
            
    '''    
    def siteSpacing(self, sender, e, displayArray):
        displayOut = 'MRN \t\tQA Unit \t\tMeasurement Date \t\tLinac Type \tBeam Set Index \tTreatment Site\tBeam Energy \tDose/Fraction \tArc 1 MU \tArc 2 MU \tArc 3 MU \tArc 4 MU\n'
        i = 0
        x = 0
        newLine = False
        ...
        for item in displayArray:
            if i < 13:
                if afm.string_width_height(str(item))[0] < 3500.0:
                    displayOut = displayOut + str(item) + '\t\t'
                elif afm.string_width_height(str(item))[0] > 18000.0:
                    displayOut = displayOut + str(item) + '\t'
                    displayOut = displayOut[:77] + '\t\t\t\t' + displayOut[77:]
                    x = 64
                elif afm.string_width_height(str(item))[0] > 14500.0:
                    displayOut = displayOut + str(item) + '\t'
                    displayOut = displayOut[:77] + '\t\t\t' + displayOut[77:]
                    x = 48
                elif afm.string_width_height(str(item))[0] > 11000.0:
                    displayOut = displayOut + str(item) + '\t'
                    displayOut = displayOut[:77] + '\t\t' + displayOut[77:]
                    x = 32
                elif afm.string_width_height(str(item))[0] > 7500.0:
                    displayOut = displayOut + str(item) + '\t'
                    displayOut = displayOut[:77] + '\t' + displayOut[77:]
                    x = 16
                else:
            ...
            if item == '\n' or '\n' in str(item):
                ...
                newLine = True
        if newLine:
            displayOut = displayOut[:77] + displayOut[81:]
        displayOut = displayOut + '\n-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------'
        if x == 64:
            displayOut = displayOut + '----------------------------------------'
        elif x == 48:
...
class Measurements:
    def __init__(self, MRN, site):
        ...
        # if dictionary is string, there is an exceptional case to be handled further in above functions and the program skips array creation
        if type(self.dictionary) == str:
            ...
            self.passrateList = []  # this will be a list of the passrates, it is used for the image display captions in getPDFimages
            self.keyList = []       # this will be a list of the beam set indexes, it is used for the image display captions in getPDFimages
            self.qaDictList = []    # this will be a list of the QA fields, sorted by beam set index alphanumerically
            ...
            for QAfield in self.qaDictList: # goes through each QA field in order of field indexes
                ...
                # this is only done on the first iteration of this loop
                ...
                if self.dataArray[fieldLabelIndex] != QAfield['fieldLabel'][0]: # if the field letter of the QA field being run doesn't match the letter for the current row, add a new row
                    ...
                    # setting object indexes based on the length of the list - this is because the length of the list depends on both the number of rows and number of passrates in a row
                    fieldLabelIndex = len(self.dataArray) - 8
                    beamSetIndex = len(self.dataArray) - 4
                    ...
                    # changing beamSetIndex each iteration bc as a beam meter set is added, the next one must be placed in the next spot in the array, so the index must increase by 1 each time
...
class Measurements2: # for MRL cases
    def __init__(self, MRN, planName):
        # retrieving QAfields & ACdictionary
        self.QAfields = query_mosaiq(MRN)
        self.fieldIndexes = []
        for QAfield in self.QAfields:
            self.fieldIndexes.append(QAfield['fieldLabel'])
        self.dictionary = getArcCheck2(MRN, planName, self.fieldIndexes)
...
def app_thread():
    # print(TestWindow().testwindow)
    #Application.Run(TestWindow().testwindow) # TargetException : Object does not match target type.
    
    thr = Thread.CurrentThread
    print('Thread ID in app_thread2(): ', thr.ManagedThreadId)
    dialog = TestWindow().testwindow # how can we access dialog inside the TestWindow class or the event handler?
    print(dialog.DialogResult)
    dialog.ShowDialog()
